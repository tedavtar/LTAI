...
public class trialBot3 : MonoBehaviour, Brain {
	...
	public string movePiece(string[] spaces){
		string rtn = "M";//should not return this, returing since graph is connected, will happen in the while loop
		
		List<string> reachableSpacesCopy = new List<string> (spaces);
		
		string myHome = "H";
		if (me.ID == 1){myHome = "F11";}
		if (me.ID == 2){myHome = "F21";}
		if (me.ID == 3){myHome = "F31";}
		
		HashSet<string> visitedTiles = new HashSet<string> ();
		Queue<Tile> fringe = new Queue<Tile> ();
		
		Tile t = GameObject.Find (myHome).GetComponent<Tile> ();
		
		visitedTiles.Add (t.ID);
		fringe.Enqueue (t);
		
		while (true) {
			Tile testTile = fringe.Dequeue();
			string testTileID = testTile.ID;
			
			if (reachableSpacesCopy.Contains(testTileID)){
				return testTileID;
			}
			...
			foreach(Tile neighborTile in neighborTiles){
				...
				if (!visitedTiles.Contains(testID)){
					visitedTiles.Add (testID);
					fringe.Enqueue (neighborTile);
				}
			}
